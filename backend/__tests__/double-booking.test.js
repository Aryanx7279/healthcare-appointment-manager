...
describe('Double-Booking Prevention', () => {
    ...
    beforeAll(async () => {
        ...
        const spec = await (0, helpers_1.createTestSpecialization)('General Medicine');
        const { profile } = await (0, helpers_1.createTestDoctor)(spec.id);
        doctorProfile = profile;
        const p1 = await (0, helpers_1.createTestPatient)();
        patient1User = p1.user;
        patient1Profile = p1.profile;
        const p2 = await (0, helpers_1.createTestPatient)();
    ...
    test('Sequential bookings - second booking should fail', async () => {
        const booking1 = await appointment_service_1.appointmentService.bookAppointment({
        ...
        expect(booking1.status).toBe(client_1.AppointmentStatus.CONFIRMED);
        // Second booking for same slot should fail
        await expect(appointment_service_1.appointmentService.bookAppointment({
            doctorId: doctorProfile.id,
    ...
    test('Concurrent bookings - exactly ONE must succeed', async () => {
        ...
        const patients = await Promise.all(Array.from({ length: 5 }).map(() => (0, helpers_1.createTestPatient)()));
        // Fire 5 simultaneous booking requests
        const promises = patients.map(({ user }) => appointment_service_1.appointmentService
            .bookAppointment({
            doctorId: doctorProfile.id,
        ...
        console.log(`Concurrent booking results: ${successes.length} success, ${failures.length} failures`);
        ...
        failures.forEach((f) => {
            expect(f.error).toMatch(/SLOT_ALREADY_BOOKED|CONFLICT|P2002/i);
        });
    ...
    test('Cancelled appointment frees the slot', async () => {
        ...
        // Book the slot
        const booking = await appointment_service_1.appointmentService.bookAppointment({
            doctorId: doctorProfile.id,
            patientUserId: patient1User.id,
            date: CANCEL_DATE,
        ...
        // Cancel it
        await appointment_service_1.appointmentService.cancelAppointment(booking.id, patient1User.id, 'Changed plans', 'PATIENT');
        // Now patient2 should be able to book the same slot
        const rebooking = await appointment_service_1.appointmentService.bookAppointment({
            doctorId: doctorProfile.id,
            patientUserId: patient2User.id,
            date: CANCEL_DATE,
            startTime: CANCEL_TIME,
        ...
        expect(rebooking.status).toBe(client_1.AppointmentStatus.CONFIRMED);
    });
});
//# sourceMappingURL=double-booking.test.js.map